...
local db_path = "../dict_db_waterway_to_confluence.sqlite"


-- Load the lsqlite3 module
...
function init_function()
	...
	-- 	local values = {}
	-- 	for value in stripped:gmatch("[^,]+") do
	-- 		table.insert(values, value)
	-- 	end
	
	-- 	CONFLUENCES[values[1]] = values[2]
	-- end
...
function get_waterway_confluence(waterway_id)
	local statement = db:prepare("SELECT value FROM data WHERE key = ?")
	statement:bind(1, waterway_id)
...
function node_function(node)
	...
	if place ~= "" then
		...
		elseif place == "village"       then mz=11
	...
	-- if natural == "lake" or natural == "sea" or natural == "ocean" or natural == "river" then
...
landcoverKeys   = { wood="wood", forest="wood",
                    ...
                    glacier="ice", ice_shelf="ice", rock="rock",
...
function way_function(way)
	...
	if isBoundary then --and not (way:Find("maritime")=="yes") then
		local mz = 3
		if     admin_level>=3 and admin_level<5 then mz=4
		elseif admin_level>=5 and admin_level<7 then mz=8
		elseif admin_level==7 then mz=13
		elseif admin_level>=8 then mz=14
		end
	...
	if highway~="" then
		...
		if majorRoadValues[highway] then              minzoom = 7 end
		if highway == "trunk"       then              minzoom = 9
		elseif highway == "primary" then              minzoom = 8 end
		if mainRoadValues[highway]  then              minzoom = 10 end
		if midRoadValues[highway]   then              minzoom = 12 end
		if minorRoadValues[highway] then h = "minor"; minzoom = 13 end
		...
		if minzoom <= 14 then
			...
			if ref~="" and (highway == "motorway" or highway == "trunk") then
	...
	if waterwayClasses[waterway] and not isClosed then
		...
		if way:Find("intermittent")=="yes" then
			...
			way:AttributeNumeric("intermittent", 0)
		end
		way:Attribute("class", waterway)
		way:Attribute("id", way:Id())
		-- add the calculated confluences
		-- local confluence = CONFLUENCES[tostring(way:Id())]
		local confluence = get_waterway_confluence(tonumber(way:Id()))
		if confluence ~= nil then
			way:Attribute("confluence", confluence)
		end
		SetNameAttributes(way)
		SetBrunnelAttributes(way)
...
function attribute_function(attr,layer)
	...
	elseif layer=="world_rivers" then
		return { class="river" }
	else
		return { class="ocean" }
	end
...
function SetMinZoomByArea(way)
	local area=way:Area()
	-- if	   area>ZRES0^2  then way:MinZoom(1)
	-- elseif area>ZRES1^2  then way:MinZoom(2)
	-- elseif area>ZRES2^2  then way:MinZoom(3)
	-- elseif area>ZRES3^2  then way:MinZoom(4)
	if area>ZRES4^2  then way:MinZoom(1)
	elseif area>ZRES5^2  then way:MinZoom(6)
